 ...
 * Description:
 */
class MakePurchaseNoteUseCase @Inject constructor(
    ...
    suspend operator fun invoke(purchaseNote: PurchaseNote): Flow<Result<PurchaseNote>> = flow {
        ...
        try {
            val madePurchaseNote = purchaseNoteRepository.insertPurchaseNote(purchaseNote)
            emit(Result.Success(madePurchaseNote))
        } catch (e: Exception) {
